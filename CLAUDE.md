...
- `src/obsidian/image-fetcher-setting-tab.ts` — settings UI, split into four `.setHeading()` sections. **General**: text inputs for the URL and image frontmatter property names and a User-Agent input. **Image Name**: toggles to name Instagram and Threads images by username. **Instagram**: a `SecretComponent` (`addComponent`) for selecting/creating the Instagram cookie secret in Obsidian's secret storage and a numeric auto-scroll count input. **Debug**: a toggle to enable logs.
