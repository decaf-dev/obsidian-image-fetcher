...
- Tests live in `test/` (e.g. `test/http-utils.test.ts`). Scope is the **pure** logic only — `isInstagramHost`, `imageNameFromUrl`, `buildRequestHeaders`, `bestFromSrcset`, `bestFromImg`, `describeHeaders`, `createDebugLogger`, `extensionFromUrl`. The Obsidian-bound paths (`fetchImagesFromUrl`, `saveImageToNote`) are not covered.
