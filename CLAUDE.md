...
- `src/obsidian/image-fetcher-setting-tab.ts` — settings UI (mirrors the social-media-scraper format): text inputs for the URL and image frontmatter property names, a toggle to name Instagram images by username, a User-Agent input, a debug toggle, and a `SecretComponent` (`addComponent`) for selecting/creating the Instagram cookie secret in Obsidian's secret storage.
